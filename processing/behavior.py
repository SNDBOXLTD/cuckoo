...
class ExtractScripts(BehaviorHandler):
    ...
    def handle_event(self, process):
        log.debug("ExtractScripts: handle_event")
        log.debug("ExtractScripts: process: \n[%s]", str(process))
        command = self.scr.parse_command(process["command_line"])
        log.debug("ExtractScripts: command: [%s]", str(command))
        if command and command.get_script():
            log.debug("ExtractScripts: command.get_script: [%s]", str(command))
    ...
    def run(self):
        ...
        for h in handlers:
            for event_type in h.event_types:
                ...
                log.debug("event_type: [%s]", str(event_type))
                ...
                if hasattr(h, "handle_%s_event" % event_type):
                    ...
                    log.debug("handler [%s] has event handle_[%s]_event", str(h), str(event_type))
                elif h.handle_event not in interest_map[event_type]:
                    interest_map[event_type].append(h.handle_event)
                    log.debug("elif handler.handle_event [%s] has event [%s]", str(h), str(event_type))

        log.debug("interest_map: [%s]", str(interest_map))
        ...
        for path in self._enum_logs():
            for handler in handlers:
                ...
                if not handler.handles_path(path):
                    log.error("handler [%s] cant handle path: [%s]", str(handler), str(path))
                    continue

                log.debug("handler [%s] handle path: [%s]", str(handler), str(path))
                # ... and then let it parse the file
                for event in handler.parse(path):
                    # pass down the parsed message to interested handlers
                    log.debug("event [%s] handler: [%s]", str(event), str(path))
                    for hhandler in interest_map.get(event["type"], []):
                        log.debug("hhandler [%s]", str(hhandler))
                        res = hhandler(event)
                        ...
                        for subevent in res:
                            log.debug("subevent: [%s]", str(subevent))
