...
class BsonParser(ProtocolHandler):
    ...
    def __iter__(self):
        ...
        while True:
            ...
            if len(data) != 4:
                log.critical("BsonParser lacking data [1].")
                log.critical("[%d] - [%s].", len(data), str(data))
                return
            ...
            if len(data) < blen:
                log.critical("BsonParser lacking data [2].")
                log.critical("[%d/%d] - [%s].", len(data), blen, str(data))
                return
            ...
            #print parsed
            #import traceback
            #log.error(str(traceback.format_stack()))
            #log.error("%s", str(dec))
