...
class ResultHandler(SocketServer.BaseRequestHandler):
    ...
    def read_any(self):
        ...
        #log.warning("read_any: [%s]", str(tmp))
    ...
    def read_newline(self, strip=False):
        ...
        log.warning("read_newline: [%s]", str(buf))
        if strip:
            buf = buf.strip()
    ...
    def negotiate_protocol(self):
        ...
        log.info("Negotiate protocol: [%s][%s]", str(protocol), command)
        ...
        try:
            ...
            for event in self.protocol:
                if isinstance(self.protocol, BsonParser) and event["type"] == "process":
                    #log.warning("event: [%s]", str(event))
                    self.open_process_log(event)
                else:
                    #log.error("ELSE event: [%s]", str(event))
                    pass
